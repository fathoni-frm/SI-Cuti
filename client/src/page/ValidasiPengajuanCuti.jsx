...
const ValidasiPengajuanCuti = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const res = await axiosPublic.get(`/validasi/pengajuan/${id}`);
				setData(res.data);
				setStatus("success");
			} catch (err) {
				setStatus("error");
	...
	return (
		<div className="max-w-3xl mx-auto mt-10 bg-white border border-gray-300 p-8 rounded-lg shadow-lg">
			{/* Header */}
			...
			<div className="mb-8">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-800">
					<p>
						<strong>Nama :</strong> {data.Pegawai?.nama}
					</p>
					<p>
						<strong>NIP :</strong> {data.Pegawai?.nip}
					</p>
					<p>
						<strong>Jabatan :</strong> {data.Pegawai.jabatanFungsional}
					</p>
					<p>
						<strong>Tanggal :</strong> {formatTanggal(data.tanggalPengajuan)}
					</p>
				</div>
			</div>
			...
			<div className="mb-6">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-800">
					...
					<p>
						<strong>Nama Pegawai :</strong> {data.Pegawai?.nama}
					</p>
					<p>
						<strong>NIP Pegawai :</strong> {data.Pegawai?.nip}
					</p>
					<p>
						<strong>Jenis Cuti :</strong> {data.jenisCuti}
					</p>
				</div>
				<div className="my-4">
					<p>
						<strong>Tanggal Cuti :</strong>{" "}
						{formatTanggal(data.tanggalMulai, false)} s.d.{" "}
						{formatTanggal(data.tanggalSelesai, false)}
					</p>
				</div>
			</div>
...
export default ValidasiPengajuanCuti;
