...
const getDataPermohonanCutiAtasan = async (req, res) => {
  try {
      ...
      include: [
        {
          ...
          where: {
            status: {
              [Op.in]: ["Diproses", "Disetujui", "Ditolak", "Dibatalkan"],
            },
          },
          include: [
            {
    ...
    for (const row of verifSaya) {
      ...
      const pelimpahanOk =
        !peng.PelimpahanTuga || peng.PelimpahanTuga.status === "Disetujui";
...
const updateStatusToDiproses = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Status verifikasi diperbarui", data: verifikasi });
  } catch (error) {
  ...
  try {
    ...
    if (statusVerifikasi === "Ditolak") {
      ...
      const pengajuan = await PengajuanCuti.findByPk(verifikasi.idPengajuan, {
        include: [
          { model: Pegawai, as: "pegawai" },
          { model: PelimpahanTugas, required: false },
        ],
      });
    ...
    return res
      .status(200)
      .json({ msg: "Status verifikasi berhasil diperbarui" });
  } catch (error) {
    res.status(500).json({ msg: error.message });
  }
};
...
const batalCutiOlehAdmin = async (req, res) => {
  try {
    ...
    if (pengajuan.status !== "Disetujui") {
      return res
        .status(400)
        .json({ msg: "Cuti hanya bisa dibatalkan jika statusnya Disetujui" });
    }
