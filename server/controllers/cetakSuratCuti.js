...
const generateNomorSurat = async () => {
  ...
  if (!konfigurasi) throw new Error("Konfigurasi sistem belum diset");
  ...
  if (konfigurasi.resetBulanan) {
    const lastUpdatedMonth = konfigurasi.updatedAt.getMonth() + 1;
    if (lastUpdatedMonth !== now.getMonth() + 1) {
  ...
  const nomorFormat = konfigurasi.formatNomorSurat
...
const generateSuratCuti = async (idPengajuan) => {
  try {
    const pengajuan = await PengajuanCuti.findByPk(idPengajuan, {
      include: [
        { model: Pegawai, as: "pegawai", include: [{ model: KuotaCuti }] },
        {
          ...
          required: false,
          include: [{ model: Pegawai, as: "penerima" }],
        },
        {
    ...
    pengajuan.nomorSurat = nomorSurat;
    ...
    const filteredVerifikator = await Promise.all(
      pengajuan.VerifikasiCutis.filter((v) =>
        ["Kepala Satuan Pelayanan", "Ketua Tim", "Kepala Bagian Umum"].includes(
          v.jenisVerifikator
        )
      ).map(async (v) => {
    ...
    if (verifikatorKepalaBalai) {
      ...
      kepalaBalai = {
        ...
        tanggal: new Date(verifikatorKepalaBalai.updatedAt).toLocaleDateString(
          "id-ID",
          formatTanggal
        ),
        qrCodePMC,
    ...
    const data = {
      ...
      tanggalMulai: new Date(pengajuan.tanggalMulai).toLocaleDateString(
        "id-ID",
        formatTanggal
      ),
      tanggalSelesai: new Date(pengajuan.tanggalSelesai).toLocaleDateString(
        "id-ID",
        formatTanggal
      ),
      kuota,
      tanggalPengajuan: new Date(pengajuan.tanggalPengajuan).toLocaleDateString(
        "id-ID",
        formatTanggal
      ),
    ...
    const templatePath = path.join(
      __dirname,
      "../uploads/template/template_surat_cuti.hbs"
    );
