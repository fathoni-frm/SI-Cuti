...
const getDaftarKepalaBalai = async (req, res) => {
  try {
    const daftarKepalaBalai = await Pegawai.findAll({
      where: {
        jabatanStruktural: { [Op.like]: "%Kepala Balai%" },
        deletedAt: null,
      },
      attributes: [
        "id",
    ...
    res.status(200).json(daftarKepalaBalai);
  } catch (error) {
    console.error(error);
    res.status(500).json({ msg: "Gagal mengambil daftar Kepala Balai." });
  }
};

const getDaftarKepalaBagianUmum = async (req, res) => {
  try {
    const daftarKabagUmum = await Pegawai.findAll({
      where: {
        jabatanStruktural: { [Op.like]: "%Kepala Bagian Umum%" },
        deletedAt: null,
      },
      attributes: [
        "id",
    ...
    res.status(200).json(daftarKabagUmum);
  } catch (error) {
    console.error(error);
    res.status(500).json({ msg: "Gagal mengambil daftar Kepala Bagian Umum." });
  }
};
...
const getDaftarKetuaTim = async (req, res) => {
  try {
    const daftarKetuaTim = await Pegawai.findAll({
      where: {
        jabatanStruktural: { [Op.like]: "%Ketua Tim%" },
      },
      include: [
        {
          model: User,
...
const getDaftarKepalaSapel = async (req, res) => {
  try {
    const daftarKepalaSapel = await Pegawai.findAll({
      where: {
        jabatanStruktural: { [Op.like]: "%Kepala Satuan Pelayanan%" },
      },
      include: [
        {
          model: User,
      ...
      include: [
        {
          ...
          where: {
            role: { [Op.notIn]: ["Admin"] },
            idPegawai: { [Op.ne]: idPegawaiLogin },
          },
          attributes: [],
        },
      ],
...
const updatePegawai = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Data pegawai dan akun berhasil diperbarui" });
  } catch (err) {
...
const importPegawai = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ msg: "Gagal memproses file", error: error.message });
  }
};
...
const cetakProfilPegawai = async (req, res) => {
  try {
    ...
    const templatePath = path.join(
      __dirname,
      "../uploads/template/profil_pegawai.hbs"
    );
...
module.exports = {
  getAllPegawai,
  getPegawaiById,
  getDaftarKepalaBalai,
  getDaftarKepalaBagianUmum,
  getDaftarKetuaTim,
