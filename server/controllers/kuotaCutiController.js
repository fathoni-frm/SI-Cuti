...
const getKuotaCutiByUser = async (req, res) => {
  try {
    ...
    const kuota = await KuotaCuti.findAll({
      ...
      include: {
        model: Pegawai,
        attributes: ["nama", "nip", "jenisKelamin"],
      },
    });

    kuotaFiltered = kuota.filter((k) => {
      return !(
        k.jenisCuti === "Cuti Melahirkan" &&
        k.Pegawai.jenisKelamin !== "Perempuan"
      );
    });

    res.status(200).json(kuotaFiltered);
  } catch (error) {
...
const createKuotaCuti = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ msg: "Kuota cuti berhasil ditambahkan", data: newKuotas });
  } catch (error) {
    console.error(error);
    res.status(500).json({ msg: error.message });
  }
...
const updateKuotaCuti = async (req, res) => {
  try {
    ...
    await Notifikasi.create({
      ...
      pesan:
        "Kuota cuti Anda telah diperbarui oleh Admin. Harap cek kuota cuti Anda.",
    });
...
const tambahKuotaCutiTahunan = async (req, res) => {
  try {
    ...
    if (!idPegawai || !jumlah) {
      return res
        .status(400)
        .json({ message: "Pegawai dan jumlah wajib diisi" });
    }
