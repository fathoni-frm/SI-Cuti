...
const getRiwayatCutiByPegawai = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ msg: "Gagal mengambil riwayat cuti", error: error.message });
  }
};
...
const getDraftCutiByPegawai = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ msg: "Gagal mengambil draft pengajuan", error: error.message });
  }
};
...
const getDraftById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ msg: "Gagal mengambil draft", error: error.message });
  }
};
...
const createPengajuanCuti = async (req, res) => {
  try {
    ...
    const statusVerifikasi =
      isDraft === "true" ? "Draft" : "Belum Diverifikasi";
    ...
    const konfigurasi = await KonfigurasiSistem.findOne({
      include: [
        { model: Pegawai, as: "kepalaBalai", attributes: ["id", "nama"] },
        { model: Pegawai, as: "kepalaBagianUmum", attributes: ["id", "nama"] },
      ],
    });
    ...
    if (!konfigurasi) {
      return res
        .status(400)
        .json({ msg: "Konfigurasi sistem belum diatur oleh admin" });
    }

    if (konfigurasi.idKepalaBagianUmum) {
      await VerifikasiCuti.create({
        idPengajuan: pengajuanId,
        idPimpinan: konfigurasi.idKepalaBagianUmum,
    ...
    if (konfigurasi.idKepalaBalai) {
      await VerifikasiCuti.create({
        idPengajuan: pengajuanId,
        idPimpinan: konfigurasi.idKepalaBalai,
    ...
    res
      .status(201)
      .json({ msg: "Pengajuan cuti berhasil dibuat", data: pengajuan });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({ msg: "Gagal membuat pengajuan cuti", error: error.message });
  }
};
...
const updatePengajuanCuti = async (req, res) => {
  try {
    const { id } = req.params;
    const { tanggalMulai, tanggalSelesai, durasi, isDraft } = req.body;
    ...
    const tanggalPengajuan =
      isDraft === "true" ? null : req.body.tanggalPengajuan;
    ...
    if (!pengajuan)
      return res.status(404).json({ msg: "Pengajuan cuti tidak ditemukan" });
    if (pengajuan.status !== "Draft") {
      return res.status(400).json({
        msg: "Pengajuan cuti tidak dapat diubah karena sudah diajukan",
      });
    }
    ...
    const existingPelimpahan = await PelimpahanTugas.findOne({
    ...
    res.status(500).json({
      msg: "Pengajuan cuti gagal diperbarui / diajukan",
      error: err.message,
    });
  }
};
...
const deletePengajuanCuti = async (req, res) => {
  try {
    ...
    if (pengajuan.status !== "Draft") {
      return res.status(400).json({
        msg: "Pengajuan cuti tidak dapat dihapus karena sudah diverifikasi",
      });
    }
